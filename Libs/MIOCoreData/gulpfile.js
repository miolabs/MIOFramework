...
function unifySwiftFiles(done) {
	...
	fs.mkdirSync(dir);
	fs.recurseSync("./MIOCoreData/", ["*.swift"], function(filepath, relative, filename) {
	...
	if(fs.existsSync("./MIOCoreData/order.json")) {
		let order = JSON.parse(fs.readFileSync("./MIOCoreData/order.json", "utf8"))
		fileArr.sort((a, b) => {
...
function parseStoryBoard(done) {
	var filesArr = [];
	var pathStoryBoard = "./MIOCoreData/Base.lproj/";
...
function copyResources(done) {
	...
	const APP_PLIST_PATH = "./MIOCoreData/Info.plist";
