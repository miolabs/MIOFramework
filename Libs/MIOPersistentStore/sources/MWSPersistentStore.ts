import { UUID, NotificationCenter, NSLog, OperationQueue,  } from "foundation";
import { NSPersistentStoreRequest, NSManagedObjectID, NSRelationshipDescription, NSIncrementalStore, NSIncrementalStoreNode,
        NSFetchRequest, NSManagedObject, NSEntityDescription, NSManagedObjectContext, NSSaveChangesRequest, NSManagedObjectSet } from "coredata";
...
import { MIOCoreObjectClone } from "mio-core"
...
export class MWSPersistentStore extends NSIncrementalStore {
    ...
    loadMetadata(): Error {
        ...
        let metadata = { NSStoreUUIDKey: uuid, NSStoreTypeKey: "MWSPersistentStore" };
        this.metadata = metadata;

        return null;
    }

    executeRequest(request: NSPersistentStoreRequest, context: NSManagedObjectContext) {

        if (request instanceof NSFetchRequest) {
            this.fetchObjects(request as NSFetchRequest, context);
            return [];
        }
        else if (request instanceof NSSaveChangesRequest) {
            this.saveObjects(request as NSSaveChangesRequest, context);
            return [];
        }
        else {
    ...
    newValueForRelationship(relationship: NSRelationshipDescription, objectID: NSManagedObjectID, context?: NSManagedObjectContext) {
        ...
        let node = this.nodesByReferenceID[referenceID] as NSIncrementalStoreNode;    
        if (node == null) return;    
        ...
        if (relationship.isToMany == false) {
            ...
            if (relNode == null) {
                ...
                //NSLog("Downloading REFID: " + relRefID);
            }            
    ...
    obtainPermanentIDsForObjects(objects){
        ...
        for (let index = 0; index < objects.length; index++){
            let obj:NSManagedObject = objects[index];
    ...
    private fetchObjectWithServerID(serverID: string, entityName: string, context: NSManagedObjectContext) {
        ...
        let fetchRequest = NSFetchRequest.fetchRequestWithEntityName(entityName);
        fetchRequest.entity = NSEntityDescription.entityForNameInManagedObjectContext(entityName, context);

        let request = this.delegate.fetchRequestForWebStore(this, fetchRequest, serverID);
        if (request == null) return;

        NotificationCenter.defaultCenter().postNotification(MWSPersistentStoreDidChangeEntityStatus, entityName, {"Status" : MWSPersistentStoreFetchStatus.Downloading});

        request.execute(this, function (this:MWSPersistentStore, code:number, data:any) {
            let [result, values] = this.delegate.requestDidFinishForWebStore(this, fetchRequest, code, data);
            
            NSLog("Downloaded REFID: " + serverID);
            ...
            NotificationCenter.defaultCenter().postNotification(MWSPersistentStoreDidChangeEntityStatus, entityName, {"Status" : MWSPersistentStoreFetchStatus.Downloaded});
        });
    }

    fetchObjects(fetchRequest: NSFetchRequest, context: NSManagedObjectContext, target?, completion?) {

        if (fetchRequest.entity == null) {
            fetchRequest.entity = NSEntityDescription.entityForNameInManagedObjectContext(fetchRequest.entityName, context);
        }
    ...
    updateObjectsInContext(items, entity: NSEntityDescription, context: NSManagedObjectContext, relationships) {
        ...
        context.performBlockAndWait(this, function (this:MWSPersistentStore) {
    ...
    isEntityParentOf(entity:NSEntityDescription, parent:NSEntityDescription):boolean {
    ...
    private updateObjectInContext(values, entity: NSEntityDescription, context: NSManagedObjectContext, objectID?:NSManagedObjectID, relationshipNodes?) {
        ...
        if (entity.name != entityName) {
            objectEntity = NSEntityDescription.entityForNameInManagedObjectContext(entityName, context);
        }
        ...
        let refresh = false;
        let node:NSIncrementalStoreNode = this.nodeWithServerID(serverID, objectEntity);
        if (node == null) {
            NSLog("New version: " + entityName + " (" + version + ")");
            node = this.newNodeWithValuesAtServerID(serverID, parsedValues, version, objectEntity, objectID);
            refresh = true;
        }
        else if (version > node.version){
            NSLog("Update version: " + objectEntity.name + " (" + node.version + " -> " + version + ")");   
    ...
    private nodeWithServerID(serverID:string, entity:NSEntityDescription){    
    ...
    private newNodeWithValuesAtServerID(serverID:string, values, version, entity:NSEntityDescription, objectID?:NSManagedObjectID){        
        let referenceID = entity.name + "://" + serverID;

        let objID = objectID != null ? objectID : this.newObjectIDForEntity(entity, serverID);

        let node = new NSIncrementalStoreNode();
    ...
    private _newNodeWithValuesAtServerID(serverID:string, values, version, entity:NSEntityDescription, objectID:NSManagedObjectID) {
        if (entity.isAbstract == true) return null;
    ...
    private isSuperEntity(fromEntity:NSEntityDescription, toEntity:NSEntityDescription) : boolean {
    ...
    private updateNodeWithValuesAtServerID(serverID:string, values:any, version:number, entity:NSEntityDescription){                
        ...
        if (this.useSaveBlocks) {
            for (let key in entity.relationshipsByName) {
                let rel = entity.relationshipsByName[key] as NSRelationshipDescription;
        ...
        NSLog("Updating REFID: " + entity.name + "://" + serverID);        
        return values;
    }

    // private _updateWithValues(serverID:string, values, version, entity:NSEntityDescription) {
    //     let referenceID = entity.name + "://" + serverID;
    //     let node = this.nodesByReferenceID[referenceID] as NSIncrementalStoreNode;
    //     node.updateWithValues(values, version);
    ...
    private deleteNodeAtServerID(serverID:string, entity:NSEntityDescription){
        let referenceID = entity.name + "://" + serverID;
        delete this.nodesByReferenceID[referenceID];
        NSLog("Deleting REFID: " + referenceID);   
    ...
    private checkRelationships(values, entity:NSEntityDescription, context:NSManagedObjectContext, relationshipNodes?){                                
    ...
    private saveCount = 0;
    private saveObjects(request: NSSaveChangesRequest, context?: NSManagedObjectContext) {
    ...
    saveObjectsOnServer(insertedObjects: NSManagedObjectSet, updatedObjects: NSManagedObjectSet, deletedObjects: NSManagedObjectSet){
        ...
        for (let index = 0; index < updatedObjects.count; index++) {
            let obj = updatedObjects.objectAtIndex(index) as NSManagedObject;
        ...
        // NSLog("OPERATION: Will save");

        // op.target = this;
        // op.completion = function () {
        //     NSLog("OPERATION: Did save");
        ...
        //     // NSLog("Object " + serverID + " -> Insert " + (result ? "OK" : "FAIL") + " (" + version + ")");                     
    ...
    private checkSaveBlockRequests(schema:string) {
        ...
        MWSCache.sharedInstance().fetch(schema, (items:any[]) => {
            ...
            let url = new URL(data["url"]);//.replace("http://localhost:8090/","https://test.dual-link.com/"));
    ...
    executeSaveRequestOperation(request:MWSRequest) {
        ...
        op.target = this;
        op.completion = function (this:MWSPersistentStore) {
            NSLog("OPERATION: Did save");            
            // let [result, serverValues] = this.delegate.requestDidFinishForWebStore(this, null, op.responseCode, op.responseJSON);
            this.saveOperationDidRemove(op);
            this.saveBlockOperationCount--;
            if (op.responseCode == 200) {
                ...
                NotificationCenter.defaultCenter().postNotification(MWSPersistentStoreSaveOperationError, op.request!.transaction);
            }
        }  
    ...
    insertObjectToServer(object: NSManagedObject) {
        ...
        NSLog("OPERATION: Insert " + object.entity.name + " -> " + serverID + ":" + this.saveCount);

        op.target = this;
        op.completion = function (this:MWSPersistentStore) {
            NSLog("OPERATION: Insert " + object.entity.name + " -> " + serverID + ":" + op.saveCount + " (OK)");
    ...
    private updateObjectOnServer(object: NSManagedObject) {
        ...
        if (node == null) {
            NotificationCenter.defaultCenter().postNotification("MWSPersistentStoreError", this, {"error":"Updating object but was never cached"});
        ...
        NSLog("OPERATION: Update " + object.entity.name + " -> " + serverID + ":" + this.saveCount);

        op.target = this;
        op.completion = function (this:MWSPersistentStore) {
            NSLog("OPERATION: Update " + object.entity.name + " -> " + serverID + ":" + op.saveCount + " (OK)");
            ...
            let [result, serverValues] = this.delegate.requestDidFinishForWebStore(this, null, op.responseCode, op.responseJSON);
            let version = this.delegate.serverVersionNumberForItem(this, serverValues);            
            NSLog("Object " + serverID + " -> Update " + (result ? "OK" : "FAIL") + " (" + version + ")");
            if (version > node.version) {
                this.updateObjectInContext(serverValues, object.entity, object.managedObjectContext, object.objectID);                
            }
            
            this.saveOperationDidRemove(op);
            NotificationCenter.defaultCenter().postNotification(MWSPersistentStoreDidUpdateEntity, object, serverID);            
        }        
    }

    deleteObjectOnServer(object: NSManagedObject) {
        ...
        NSLog("OPERATION: Delete " + object.entity.name + " -> " + serverID);

        op.target = this;
        op.completion = function (this:MWSPersistentStore) {
            NSLog("OPERATION: Delete " + object.entity.name + " -> " + serverID + "(OK)");
            ...
            NSLog("Object " + serverID + " -> Deleted " + (result ? "OK" : "FAIL"));                     
            this.saveOperationDidRemove(op);
        }  
    }
    ...
    private fetchOperationQueue:OperationQueue = null;
    private fetchFromServer(){
        if (this.fetchOperationQueue == null) {
            this.fetchOperationQueue = new OperationQueue();
            this.fetchOperationQueue.init();
        }        
    }

    private _saveOperationQueue:OperationQueue = null;
    private get saveOperationQueue(): OperationQueue {
        if (this._saveOperationQueue == null) {
            this._saveOperationQueue = new OperationQueue();
            this.saveOperationQueue.init();
            //this.saveOperationQueue.maxConcurrentOperationCount = 1;  No existe esta variable en la clase ni parece haber una equivalente
        }
...
export class MWSPersistentStore extends NSIncrementalStore {
    ...
    private fetchObjectWithServerID(serverID: string, entityName: string, context: NSManagedObjectContext) {
        ...
        request.execute(this, (code, data)  => {
            let [result, values] = this.delegate.requestDidFinishForWebStore(this, fetchRequest, code, data);
    ...
    updateObjectsInContext(items, entity: NSEntityDescription, context: NSManagedObjectContext, relationships) {
        ...
        context.performBlockAndWait(this, () => {
    ...
    saveObjectsOnServer(insertedObjects: NSManagedObjectSet, updatedObjects: NSManagedObjectSet, deletedObjects: NSManagedObjectSet){
        ...
        for (let index = 0; index < insertedObjects.count; index++) {
            ...
            const values = this.delegate.serverValuesForObject(this, obj, true, "INSERT");
            const serverID = this.delegate.serverIDForObject(this, obj);
            this.newNodeWithValuesAtServerID(serverID, NSCoreObjectClone(values["values"]), 1, obj.entity, obj.objectID);
            if (this.delegate.canSynchronizeEntity(this, obj.entity, "INSERT")) items.push(values);
        }
        ...
        for (let index = 0; index < updatedObjects.count; index++) {
            ...
            let node = this.nodeWithServerID(obj.objectID._getReferenceObject(), obj.entity);
            let version = node.version + 1;
            this.updateNodeWithValuesAtServerID(serverID, NSCoreObjectClone(values["values"]), version, obj.entity);
            if (this.delegate.canSynchronizeEntity(this, obj.entity, "UPDATE")) items.push(values);
        }
    ...
    private checkSaveBlockRequests(schema:string) {
        ...
        MWSCache.sharedInstance().fetch(schema, (items:any[]) => {
            ...
            let r = new MWSJSONRequest();
            let url = URL.urlWithString(data["url"]);//.replace("http://localhost:8090/","https://test.dual-link.com/"));
            r.initWithURL(url, data["body"], data["method"]);
            r.headers = data["headers"];
            r.transaction = data["transaction"];
            r.schema = data["schema"];
