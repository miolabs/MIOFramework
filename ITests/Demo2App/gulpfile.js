...
function unifySwiftFiles(done) {
	...
	//if there's a file '/Demo2App/order.json', we expect it in the format ["file1.swift", "subfolder/file2.swift"]
	//it's a way to provide a custom order for the swift files (with proper dependency order)
	if(fs.existsSync("./Demo2App/order.json")) {
		let order = JSON.parse(fs.readFileSync("./Demo2App/order.json", "utf8"))
		fileArr.sort((a, b) => {
...
function parseDataModel(done) {
	var DATAMODEL_PATH = "./Demo2App/Demo2App" + ".xcdatamodeld/Demo2App.xcdatamodel/contents";
