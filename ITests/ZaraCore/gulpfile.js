...
function unifySwiftFiles(done) {
	...
	fs.mkdirSync(dir);
	fs.recurseSync("./ZaraCore/", ["*.swift"], function(filepath, relative, filename) {
	...
	//if there's a file '/ZaraCore/order.json', we expect it in the format ["file1.swift", "subfolder/file2.swift"]
	//it's a way to provide a custom order for the swift files (with proper dependency order)
	if(fs.existsSync("./ZaraCore/order.json")) {
		let order = JSON.parse(fs.readFileSync("./ZaraCore/order.json", "utf8"))
		fileArr.sort((a, b) => {
...
function parseStoryBoard(done) {
	var filesArr = [];
	var pathStoryBoard = "./ZaraCore/Base.lproj/";
...
function parseDataModel(done) {
	var DATAMODEL_PATH = "./ZaraCore/ZaraCore" + ".xcdatamodeld/ZaraCore.xcdatamodel/contents";
...
function copyResources(done) {
	...
	const APP_PLIST_PATH = "./ZaraCore/Info.plist";
	...
	const ASSETS_PATH = "./ZaraCore/Assets.xcassets/";
