...
function unifySwiftFiles(done) {
	...
	fs.mkdirSync(dir);
	fs.recurseSync("./Demo5/", ["*.swift"], function(filepath, relative, filename) {
	...
	//if there's a file '/Demo5/order.json', we expect it in the format ["file1.swift", "subfolder/file2.swift"]
	//it's a way to provide a custom order for the swift files (with proper dependency order)
	if(fs.existsSync("./Demo5/order.json")) {
		let order = JSON.parse(fs.readFileSync("./Demo5/order.json", "utf8"))
		fileArr.sort((a, b) => {
...
function parseStoryBoard(done) {
	var filesArr = [];
	var pathStoryBoard = "./Demo5/Base.lproj/";
...
function parseDataModel(done) {
	var DATAMODEL_PATH = "./Demo5/Demo5" + ".xcdatamodeld/Demo5.xcdatamodel/contents";
...
function copyResources(done) {
	...
	const APP_PLIST_PATH = "./Demo5/Info.plist";
